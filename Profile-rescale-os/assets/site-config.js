window.SITE = {
  name: "Rajeev Kumar",
  title: "Senior AI Product Manager, Platform & Agent Systems",
  email: "rajeevkmba2025@gmail.com",
  linkedin: "https://www.linkedin.com/in/rajeevkumar9/",
  github: "https://github.com/rajeevcode",
  portfolio: "https://rajeevcode.github.io/product-portfolio/",
  portfolioHome: "../index.html",
  rescaleOperatorCaseStudy: "../rescale-operator/creative-marketing-agents/index.html",
  pulsaraCaseStudy: "../Pulsara-Portfolio/index.html",
  roles: [
    "AI Product Manager",
    "Senior Product Manager, AI / Platform",
    "Technical Product Manager",
    "AI Automation Lead",
    "AI Workflow / Agentic Systems Builder",
    "Product Owner, AI-enabled Operations",
  ],
};
